    ...
    case TeamJoin(user: User) => responder ! PrivateResponse(
     // s"Hey ${user.name} and welcome to our community!",
      """Привет!
        ...
        |Описание каналов:
        |
        |#general - вопросы, которые остались после прочтения гайда из шапки канала #guide. Охуительные истории связанные с фрилансом и клиентами.
        |#guide - если еще не читал гайд, то стоит зайти и прочитать перед задаванием вопросов в #general
        ...
        |
        |
        |А теперь стоит поздороваться в #general, рассказать о себе (фрилансишь ли уже или собираешься, на чём, стаж и прочее, что найдешь нужным сказать).
        |
